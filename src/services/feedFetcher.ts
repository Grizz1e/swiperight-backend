import { FeedSource, Article } from '../types/index.js';
...
async function fetchSingleFeed(source: FeedSource): Promise<{ articles: Article[]; etag: string | null; skipped: boolean }> {
...
export async function fetchAllFeeds(): Promise<{ results: FetchResult[]; totalArticles: number; cleanup: number }> {
  ...
  const feeds: FeedSource[] = sourceRows.map(row => ({
    id: row.id,
    name: row.name,
  ...
  const fetchPromises = feeds.map(async (source): Promise<FetchResult> => {
    try {
