...
// Sources CRUD
export async function upsertSources(sources: FeedSource[]): Promise<void> {
...
export async function getSourceEtag(sourceId: string): Promise<string | null> {
  const db = getSupabase();
  const { data } = await db
    .from('sources')
    .select('etag')
...
export async function updateSourceFetchInfo(sourceId: string, etag: string | null): Promise<void> {
  const db = getSupabase();
  await db
    .from('sources')
    .update({ 
